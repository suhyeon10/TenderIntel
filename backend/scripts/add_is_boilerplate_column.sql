-- linkus_legal_legal_chunks 테이블에 is_boilerplate 컬럼 추가
-- 머리말/기타/boilerplate 청크를 필터링하기 위해 사용

-- 1. 컬럼 추가
ALTER TABLE linkus_legal_legal_chunks 
ADD COLUMN IF NOT EXISTS is_boilerplate boolean DEFAULT false;

-- 2. 인덱스 추가 (필터링 성능 향상)
CREATE INDEX IF NOT EXISTS idx_linkus_legal_legal_chunks_is_boilerplate 
ON linkus_legal_legal_chunks(is_boilerplate) 
...
-- UPDATE linkus_legal_legal_chunks 
