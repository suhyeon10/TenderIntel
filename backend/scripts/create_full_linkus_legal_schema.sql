-- Full legal schema bootstrap for a shared Linkus database
-- Uses the `linkus_legal_` prefix for all Linkers Legal tables and RPC functions.
-- Note: current backend code still points to the unprefixed names.
...
CREATE INDEX IF NOT EXISTS idx_linkus_legal_contract_analyses_clauses
ON public.linkus_legal_contract_analyses USING GIN (clauses);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_contract_analyses_highlighted_texts
ON public.linkus_legal_contract_analyses USING GIN (highlighted_texts);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_contract_analyses_metadata
ON public.linkus_legal_contract_analyses USING GIN (metadata);
...
CREATE TABLE IF NOT EXISTS public.linkus_legal_chat_sessions (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    ...
    CONSTRAINT linkus_legal_chat_sessions_context_type_check
        CHECK (initial_context_type IN ('none', 'situation', 'contract'))
);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chat_sessions_user_id
ON public.linkus_legal_chat_sessions(user_id);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chat_sessions_user_updated
ON public.linkus_legal_chat_sessions(user_id, updated_at DESC);

CREATE TABLE IF NOT EXISTS public.linkus_legal_chat_messages (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id UUID NOT NULL REFERENCES public.linkus_legal_chat_sessions(id) ON DELETE CASCADE,
    user_id UUID NOT NULL,
    ...
    metadata JSONB DEFAULT '{}'::jsonb,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT linkus_legal_chat_messages_sender_type_check
        CHECK (sender_type IN ('user', 'assistant', 'system')),
    CONSTRAINT linkus_legal_chat_messages_context_type_check
        CHECK (context_type IN ('none', 'situation', 'contract'))
);

CREATE UNIQUE INDEX IF NOT EXISTS idx_linkus_legal_chat_messages_session_sequence
ON public.linkus_legal_chat_messages(session_id, sequence_number);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chat_messages_session_id
ON public.linkus_legal_chat_messages(session_id, created_at);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chat_messages_metadata
ON public.linkus_legal_chat_messages USING GIN (metadata);

CREATE OR REPLACE FUNCTION public.linkus_legal_touch_chat_session_updated_at()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    UPDATE public.linkus_legal_chat_sessions
    SET updated_at = now()
...
DROP TRIGGER IF EXISTS trg_linkus_legal_touch_chat_session_updated_at
ON public.linkus_legal_chat_messages;

CREATE TRIGGER trg_linkus_legal_touch_chat_session_updated_at
AFTER INSERT ON public.linkus_legal_chat_messages
FOR EACH ROW
EXECUTE FUNCTION public.linkus_legal_touch_chat_session_updated_at();

-- ---------------------------------------------------------------------------
-- Agent traces
-- ---------------------------------------------------------------------------

CREATE TABLE IF NOT EXISTS public.linkus_legal_agent_traces (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    session_id UUID REFERENCES public.linkus_legal_chat_sessions(id) ON DELETE CASCADE,
    message_id UUID REFERENCES public.linkus_legal_chat_messages(id) ON DELETE SET NULL,
    user_id UUID,
    mode TEXT NOT NULL DEFAULT 'contract',
    context_type TEXT,
    context_id TEXT,
    contract_analysis_id UUID REFERENCES public.linkus_legal_contract_analyses(id) ON DELETE SET NULL,
    ...
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    CONSTRAINT linkus_legal_agent_traces_mode_check
        CHECK (mode IN ('plain', 'contract', 'situation')),
    CONSTRAINT linkus_legal_agent_traces_verification_status_check
        CHECK (
...
CREATE INDEX IF NOT EXISTS idx_linkus_legal_agent_traces_session_id
ON public.linkus_legal_agent_traces(session_id, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_agent_traces_contract_analysis_id
ON public.linkus_legal_agent_traces(contract_analysis_id, created_at DESC);
...
ON public.linkus_legal_contract_chunks USING GIN (metadata);
...
CREATE OR REPLACE FUNCTION public.linkus_legal_match_contract_chunks(
...
BEGIN
    ...
    SELECT
        ...
        END AS similarity,
        cc.metadata
    FROM public.linkus_legal_contract_chunks AS cc
...
CREATE TABLE IF NOT EXISTS public.linkus_legal_chunks (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_linkus_legal_chunks_external_id
ON public.linkus_legal_chunks(external_id);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chunks_source_type
ON public.linkus_legal_chunks(source_type);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chunks_title
ON public.linkus_legal_chunks(title);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chunks_metadata
ON public.linkus_legal_chunks USING GIN (metadata);

CREATE INDEX IF NOT EXISTS idx_linkus_legal_chunks_embedding
ON public.linkus_legal_chunks
USING ivfflat (embedding vector_cosine_ops)
WITH (lists = 100);

CREATE OR REPLACE FUNCTION public.linkus_legal_match_legal_chunks(
    query_embedding vector(1024),
    match_threshold FLOAT DEFAULT 0.5,
...
AS $$
    ...
    FROM public.linkus_legal_chunks AS lc
